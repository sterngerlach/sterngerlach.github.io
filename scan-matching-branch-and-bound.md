...
- キュー$\mathcal{C}$が空になるまで、以下を繰り返す。
  ...
  - ノード$\mathbf{c}$の高さが$h = 0$、言い換えると葉ノードである場合は、現在の最大値$s^*$を$\overline{s}(\mathbf{c})$、また最適解$(j_x^*, j_y^*, j_\theta^*)$を$(c_x, c_y, c_\theta)$で更新し、上に戻る。
  ...
  $$
    ...
    \end{eqnarray}
  $$
  上界の最も大きな子ノードが先頭に来るように、4つの子ノードを$\mathcal{C}$に追加していく。
...
上記のアルゴリズムだけではイメージが湧かないと思うので、Pythonによる実装を示します。
